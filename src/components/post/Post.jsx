...
function Post(){
    ...
    const {state,setState,comments,setComments,replies,getComment} = useContext(MyContext)
    ...
    function handleCommentsShowMore(id){
        if( state.data.filter(i => i.id===id)){
            var nm = parseInt(state.data.filter(i => i.id===id).map(i=>i.comments))
            setState({...state},state.data.filter(i => i.id===id).map(i=>i.no=nm))
    ...
    return(
        ...
        <div className="singlePost bg-white mb-5" styles={{ height: '500px', overflowY: 'scroll' }}>
            ...
            <div className="commentTop d-flex  justify-content between w-100 p-3">
                {row.comments > 1 &&
                <div className="commentViewMore text-muted text-left w-100 " ><div onClick={() => handleCommentsShowMore(row.id)}>view {row.comments-1} more</div></div>
                }
                {/* <div className="commentFilter text-muted text-right w-100">All comments  &#9662; </div> */}
            </div>
            }
            
            { showPostComments.includes(row.id) && comments.data.filter(i => i.post === row.id).slice(0, commentsShowingCount).map((com,index) => (
            ...
            <div className="addComment mb-4 container">
                ...
                <div className="addCommentRight">
                    <form action="" className="addCommentForm" onSubmit={(event)=>handleNewCommentSubmit(event,row.id)} method="POST" >
                        <input type="text" placeholder="Write a reply..." onChange={(e)=>handleNewCommentChange(e,row.id)} value={comments.comment} name="comment" />
                    </form>
